...
const buildDir = `${process.env.PWD}/html`;
...
const addMetadata = _edition => {
  ...
  const randobando = Math.round(Math.random() * 100);
      ...
      if (randobando <= 15){rarityForEdition = "Rare"; var hashrate = 4;}
      if (randobando <= 4){rarityForEdition = "Epic"; var hashrate = 8;}
      if (randobando <= 1){rarityForEdition = "Legendary"; var hashrate = 24;}
  ...
  hash = [];
  decodedHash = [];
  console.log("Default mint initiated with " + rarityForEdition + " rarity (#" + _edition + ")");
};
...
const createBlankMetaData = () => {
  fs.stat(`${buildDir}/${blankMetDataFile}`, (err) => {
    if(err == null || err.code === 'ENOENT') {
      var newmeta = JSON.stringify(blankmeta,null, 2).slice(1,-1)
      fs.writeFileSync(`${buildDir}/${blankMetDataFile}`, newmeta , { mode: 0o755 });
    } else {
        console.log('Oh no, error: ', err.code);
    }
  });
...
module.exports = { buildSetup, createFiles, createMetaData, createBlankMetaData};
